...
o, = plt.plot(*np.transpose(dwd.progField.return_maxima(t)[:, 2:0:-1]), 'wX')
...
t = 0
fig, ax = plt.subplots(1, figsize=(10, 8))
im = ax.imshow(dwd.nested_data[0, :, :], norm=matplotlib.colors.SymLogNorm(vmin=0, linthresh=1), cmap=cmap)
plt.show(block=False)
o, = plt.plot(*np.transpose(dwd.progField.return_maxima(t)[:, 2:0:-1]), 'wX')
...
ax.set_xticklabels(ax.get_xticks() * 0.25)
ax.set_yticklabels(ax.get_yticks() * 0.25)
matplotlib.rcParams.update({'axes.size': 20})
