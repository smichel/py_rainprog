...
\begin{equation}
	...
	\label{eq:bivariate}
\end{equation}
Now with the displacements in x and y direction from the tracks a two dimensional Gaussian distribution with \ref{eq:bivariate} is constructed. 
\begin{figure}[h]
	\centering
	\begin{tikzpicture}[
...
Once the $x$- and $y$ displacements are estimated and processed, they are applied to the precipitation fields of the DWD and PATTERN radar to perform the prognosis. The latest available DWD precipitation field is then simply displaced by the mean $x$- and $y$ displacements from \ref{subsec:disdetect}. This method is called Lagrangian persistence.\textbf{reference} Hereby the precipitation pattern does not change in itself and is merely moved by a constant factor. It is assumes that the precipitation does not change over the prognosis time. This obviously can lead to huge errors if the precipitation strengthens or weakens over time. Especially in convective situation this can cause a wrong prognosis. \\
...
\begin{equation*}
	...
	0 & 0 & 0 & 0 & 0 & 0 & 0.02& 0.07 	& 0.03 	& 0 	& 0\\
