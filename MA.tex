...
To differentiate good from bad tracks a orthogonal distance regression (ODR) (Fig. \ref{fig:orthogonaldist}) for each track is performed. The ODR calculates a line through a dataset of points which minimizes the squared distances from the points to their anchor points on the line. For simplicity the movement of the precipitation fields is assumed to be a line and not a curve. 
...
\begin{figure}[h]
	...
	\begin{axis}[
		...
		\fill (1.1,0.7) circle[radius=2pt];
		\fill (1.4,0.3) circle[radius=2pt];
		\fill (1.9,0.8) circle[radius=2pt];
		\fill (2.5,0.2) circle[radius=2pt];
		\fill (2.9,0.6) circle[radius=2pt];
		\fill (3.5,0.5) circle[radius=2pt];
		\draw (0.5,0.5) -- (3.5,0.5);
		\draw[red] (1.1,0.7) -- (1.1,0.5);
		\draw[red] (1.4,0.3) -- (1.4,0.5);		
		\draw[red] (1.9,0.8) -- (1.9,0.5);
		\draw[red] (2.5,0.2) -- (2.5,0.5);
		\draw[red] (2.9,0.6) -- (2.9,0.5);
		
	\end{axis}
	\end{tikzpicture}
	\caption{Example for the orthogonal distance regression}
