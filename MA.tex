...
The prognosis model is a combination of a linear advection model and a probabilistic nowcasting model. It is based on field based motion detection motivated by identifying local maxima in the precipitation field. \\
...
The polar coordinates of the radar data are described by the azimuth angle $azi$ and distance from the radar $r$. With \ref{eq:trans} the polar coordinates are transformed to Cartesian distances to the radar.
\begin{equation}
	\begin{array}{lcl}
		x = r \cdot \cos(azi)\\
		y = r \cdot \sin(azi)
	...
	\label{eq:trans}
\end{equation}
The Cartesian distances to the radar are then transformed to Cartesian coordinates with the Transverse Mercator projection \footnote{For the Transverse Mercator projection the 'Geospatial Data Abstraction Library' (GDAL) and the python module 'osgeo' is used \citep{Warmerdam}.}. Documentation for GDAL: 'gdal.org/python'. This is done for the coordinates of the DWD and PATTERN radars. From the DWD radar only a square with an edge length of 100\,km with the centre at the position of the PATTERN radar is used(Fig. \ref{fig:radarOverview}). This immensely reduces the computational work. \\ % hier vielleicht noch was über verlängerung der prognosezeit.
The DWD and PATTERN radar data is available in clutter corrected radar reflectivity.%Hier noch den typ der daten, CLT corrected,xband,cband
Reflectivity $z$ is a measure for the energy intensity which is backscattered by remote objects. The reflectivity $z$ is given in [mm$^{6}$m$^3$] and is proportional to the number of droplets and the size of the droplets. The reflectivity is more often used in its logarithmic form $Z$ in [dBZ] and calculated with eq. \ref{eq:dBZ}. The logarithmic form of the reflectivity is mostly used to make it easier to read.
\begin{equation}
