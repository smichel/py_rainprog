...
The latest available DWD precipitation field is then simply displaced by the mean $x$- and $y$ displacements estimated in \ref{subsec:disdetect}. This method is called Lagrangian persistence.\textbf{reference} Hereby it is assumed that the precipitation field does not change over the prognosis time and is merely advected horizontally. This obviously can lead to huge errors if the precipitation strengthens or weakens over time. Especially in convective situations this can cause faults in the prognosis. \\
...
\begin{figure}[h]
	...
	\begin{minipage}{0.48\textwidth}
		...
		\caption{Example plot with pixels of 1px, 2px and 3px width with a arbitrary value of 1, at timestep $t=0$.}
		\label{fig:extrapolation_t0}
	\end{minipage}\hfill
	\begin{minipage}{0.48\textwidth}
		\includegraphics[width=\textwidth,trim={38mm 0 10mm 5mm},clip]{Extrapolation_t1.eps}
		\caption{Example plot where the pixels from Fig. \ref{fig:extrapolation_t0} are displaced and smoothed by eq. \ref{eq:gausskernel} at timestep $t=1$.}
