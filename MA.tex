...
\begin{center}
	...
	\draw (3.5,3.3) node {$f(x,y)$};
	%1/(sqrt(pow(#2,2)*2*pi))*exp(-(pow((#3-#1),2))/(2*pow(#2,2)))
	\foreach \x in {0, ..., 15} {
		\coordinate (A) at ({invgauss(rnd,rnd)/2},{invgauss(rnd,rnd)/2});
		\draw (A) -- (3,3);
		\fill (A) circle[radius=1pt];
	}
	\draw (0.5,-0.5) --(3,3);
	\fill (0.5,-0.5) circle[radius=2pt];
	\draw (0.52,-0.8) node {$f(x_s,y_s)$};

    \end{axis}
	\end{tikzpicture}
\end{center}
\begin{equation}
	f(x,y) = \dfrac{1}{n} \Bigg(\sum_{s=0}^{n}f(x_s,y_s)\Bigg)
\end{equation}

\begin{equation}
	g(x,y) = A\, \text{exp}\Bigg(-\Bigg(\dfrac{(x-x_0)^2}{2\sigma^2_x}+\dfrac{(y-y_0)^2}{2\sigma^2_y}		\Bigg)\Bigg)
\end{equation}
